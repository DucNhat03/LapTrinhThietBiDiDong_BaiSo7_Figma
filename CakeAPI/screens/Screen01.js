...
const Screen01 = () => {
  ...
  const fetchDonuts = async (category) => {
    ...
    try {
      if (category === 'donut') {
        const responseDonut = await axios.get('http://localhost:5000/donut'); // Thay localhost bằng IP của máy
  ...
  const handleDonutPress = (item) => {
    navigation.navigate('Screen02', { donut: item }); // Chuyển hướng trực tiếp đến màn hình chi tiết
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.welcomeText}>Welcome you!</Text>
